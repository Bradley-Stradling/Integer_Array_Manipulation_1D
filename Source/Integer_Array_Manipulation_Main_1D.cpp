...
int main()
{
	...
	output_Array_1D(array_Of_Numbers, array_Size);
	cout << endl;
	...
	do// start of  main menu loop
	{
		...
		do
		{
			flag = 0;
			cin >> selection;
			flag = integer_Input_Error_Check(selection, selection_Limit_Min,
		...
		if (selection == 1)
		{
			shift_Right_1D(array_Of_Numbers, array_Size);
			cout << endl;
		}

		if (selection == 2)
		{
			shift_Left_1D(array_Of_Numbers, array_Size);
			cout << endl;
		}

		if (selection == 3)
		{
			total = array_Total_1D(array_Of_Numbers, array_Size);
		...
		if (selection == 4)
		{
			...
			do
			{
				flag = 0;
				cin >> address;
				flag = integer_Input_Error_Check(address, selection_Limit_Min,
					selection_Limit_Max, flag);
			} while (flag == 1);

			single_Element_1D(array_Of_Numbers, address);
			cout << endl;
		}

		if (selection == 5)
		{
			average = array_Average_1D(array_Of_Numbers, array_Size);
		...
		if (selection == 6)
		{
			low_To_High_1D(array_Of_Numbers, array_Size);
			cout << endl;
		}

		if (selection == 7)
		{
			high_To_Low_1D(array_Of_Numbers, array_Size);
			cout << endl;
		}

		if (selection == 8)
		{
			randomize_Array_1D(array_Of_Numbers, array_Size);
			cout << endl;
		}
